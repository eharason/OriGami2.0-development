...
.controller('TeacherCtrl', ['$rootScope', '$scope', '$timeout', '$ionicModal', '$window', '$ionicHistory',
    ...
    API.getAll().success(function (data, status, headers, config) {
        ...
        if ($scope.list.length === 0) {
            ...
            for (var ii = 0; ii < $scope.list.length; ii++) {
                $scope.list[ii].diff = Array.apply(null, Array($scope.list[ii].difficulty)).map(function () {
                    return "ion-ios-star";
                });
            }
            $scope.noData = false;
...
.controller('PlayCtrl', ['$scope', '$stateParams', '$ionicModal', '$ionicPopup', '$ionicLoading', '$location', '$cordovaSocialSharing',
    ...
    $scope.showWaypointInfoModal = function() {
     ...
     var handleNextWaypoint = function () {
       // TODO: change back! Get all waypoints!
    ...
    // $scope.closeModal();
    // createModal('templates/tasks/chart.html');
 ...
 */
 .controller('matchCtrl', ['$scope', '$rootScope', '$cordovaGeolocation', '$stateParams', '$ionicModal', '$ionicLoading', '$timeout', 'leafletData', '$translate', 'PathData', '$compile', function($scope, $rootScope, $cordovaGeolocation, $stateParams, $ionicModal, $ionicLoading, $timeout, leafletData, $translate, PathData, $compile) {
         ...
         // once populated, check if rows are good
         ...
         //pie chart happiness
         $scope.labels = ['Black', 'Green', 'Yellow', 'Blue'];
         $scope.data = [
           [90,90,90,90]
         ];
         $scope.datasetOverride = [
           {
             backgroundColor: ["#333333","#31a354","#FDB45C","#2b8cbe"],
             hoverBackgroundColor: ["#4d4d4d","#3bc464","#fec581","#44a4d5"],
             type: 'pie'
           }
         ];
